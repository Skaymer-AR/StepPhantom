...
/**
 * Lado HOOK. Resuelve la config con cascada robusta. El default seguro es
 * "config vacía" -> el hook NO simula nada y deja pasar los valores originales.
 * Nunca inventamos valores falsos por defecto en silencio.
 *
 *   1) ContentProvider (principal, independiente de versión; requiere Context).
 *   2) XSharedPreferences (fallback; depende del bridge de LSPosed/Vector).
 *   3) StepPhantomConfig() vacío (seguro).
 */
object HookConfigSource {
    ...
    private fun queryPrefs(): StepPhantomConfig? = try {
        ...
        xsp.reload()
        if (!xsp.file.canRead()) null
        else xsp.getString(StepPhantomConfig.PREFS_KEY_JSON, null)
            ?.let { StepPhantomConfig.fromJsonString(it) }
    } catch (t: Throwable) {
